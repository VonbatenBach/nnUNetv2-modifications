...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    if save_probabilities:
        ...
        ### Here is a custom modification, which allows to save probability map in the same format
        # as segmentation maps. Important: dataset must be in format read by SimpleITK IO ###
        rw = plans_manager.image_reader_writer_class()
        rw.write_seg(probabilities_final[1], output_file_truncated + '_prob_ch1' + dataset_json_dict_or_file['file_ending'],
                     properties_dict, probabilities = True)
        del probabilities_final, ret, rw
    else:
